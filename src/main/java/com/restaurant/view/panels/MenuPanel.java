...
public class MenuPanel extends JPanel {
    ...
    private int currentCategoryId = -1; // để nhớ danh mục đang chọn
    private int currentPage = 0;
    private int itemsPerPage = 9; // 3x3
    private List<Dish> currentDishes; // danh sách món hiện tại
    ...
    private void showDishesByCategory(int categoryId) {
        ...
        txtSearch.setText(""); // reset tìm kiếm
    ...
    private void filterDishesByName() {
        ...
        if (keyword.isEmpty()) {
            if (currentCategoryId != -1) {
                ...
                // Nếu chưa chọn danh mục, hiện tất cả món
        ...
        // Lọc từ tất cả món (hoặc từ danh mục hiện tại)
        List<Dish> allDishes;
        if (currentCategoryId != -1) {
    ...
    private JPanel createDishCard(Dish dish) {
        ...
        if (icon == null) {
            // Placeholder
            icon = new ImageIcon(getClass().getResource("/images/default-dish.png"));
            if (icon == null) {
        ...
        JButton orderBtn = new JButton("Đặt món");
        ...
        UIHelper.applyHoverEffect(orderBtn,
                AppColors.PRIMARY,
                new Color(100,160,210));

        orderBtn.addActionListener(e -> showQuantityDialog(dish));
    ...
    private void showQuantityDialog(Dish dish) {
        String input = JOptionPane.showInputDialog(
                this,
                "Nhập số lượng cho món " + dish.getName(),
                "Chọn số lượng",
        ...
        try {
            ...
            mainFrame.addToCart(dish, quantity);
            JOptionPane.showMessageDialog(this,
                    "Đã thêm " + dish.getName() + " x" + quantity + " vào giỏ");
        } catch (Exception e) {
